...
(define (cookbook-header header-file-path #!optional (outport
						      (current-output-port)))
...
;;; opened already, and closed by another procedure later
(define (cookbook-footer footer-file-path #!optional (outport (current-output-port)))
...
(define (ing-to-latex myrecipe #!optional (outport (current-output-port)))
...
(define (prep-to-latex myrecipe #!optional (outport (current-output-port)))
  (let ((prep (cdr (assoc "prep" myrecipe))))
...
(define (hint-to-latex myrecipe #!optional (outport (current-output-port)))
...
(define (cookbook-recipe myrecipe #!optional (outport (current-output-port)))
  (write-line "\\newpage" outport)
...
(define (recipes-to-latex recipes #!optional (outport (current-output-port)))
